...
def gibbs_sampler_gmm_multivariate(X, K, num_iterations, burn_in):
    ...
    # -- Hyperparameters --
    alpha = np.ones(K) * 5     # Dirichlet prior for pi #larger alpha means more uniform distribution
    ...
    S0 = np.cov(X.T) * 2      # Scale matrix for Inv-Wishart #larger S0,more spread out
    ...
    for iteration in tqdm(range(num_iterations), desc="Sampling"):
        ...
        log_posterior = np.log(np.clip(pi, 1e-10, None)) + np.array(
            [multivariate_normal.logpdf(X, mean=mu[k], cov=Sigma[k]+ 1e-6 * np.eye(p))for k in range(K)]
        ).T
        ...
        for k in range(K):
            ...
            if n_k > 0:
                ...
                Sigma_k_inv = np.linalg.pinv(Sigma[k])
...
def bayesian_repulsive_randomwalk(X, K, num_iterations, h, burn_in, sig, mu):
    """
...
def bayesian_repulsive_randomwalk_joint(X, K, num_iterations, h, burn_in):
    ...
    for iteration in tqdm(range(num_iterations), desc="Sampling"):
        ...
        log_target_current += np.log(max(h(mu), 1e-10))
